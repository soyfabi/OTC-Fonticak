...
function Store:setCreatureOutfit(widget, outfit)
	if not widget then
		return
	end
	...
	if widget.setCreatureSize then
		local size = widget.getHeight and widget:getHeight() or 64
		if size <= 0 then
			size = 64
		end
		widget:setCreatureSize(size)
	end
	widget:setOutfit(outfit or {})
	if widget.enableStorePreviewAnimation then
		widget:enableStorePreviewAnimation()
	elseif widget.setAnimate then
		widget:setAnimate(true)
		if widget.setIdleAnimate then
			widget:setIdleAnimate(true)
		end
	elseif widget.getCreature then
...
-- Fonticak Cip API is 4-arg; Fantoner storeprotocol replaces requestStoreOffers on init.
-- Do not wrap the native binding with Astra's 2-arg helper.
...
local localImageAliases = {
	["13/xp_boost"] = "/images/dailyreward/icon-reward-xpboost",
...
function Store:downloadImage(requestId, image, disabled, onLoaded)
	...
	-- Fantoner has no CDN; never treat relative paths like "64/xp_boost" as hostnames.
...
-- Horizontal slide between two sibling widgets. direction: 1 = next (to left), -1 = prev (to right).
function Store:safeSlidePair(outgoing, incoming, direction, duration, onFinish)
	if not isWidgetAlive(outgoing) or not isWidgetAlive(incoming) then
		if onFinish then
			onFinish(false)
		end
		return
	...
	duration = math.max(1, duration or 240)
	local width = math.max(1, outgoing:getWidth())
	local startOut = outgoing:getMarginLeft()
	local startIn = direction * width
	incoming:setMarginLeft(startIn)
	incoming:setOpacity(1)

	local endOut = startOut - (direction * width)
	local endIn = 0
	...
	Store:safeCancelWidgetAnimations(outgoing)
	Store:safeCancelWidgetAnimations(incoming)
	animateValue(outgoing, "slide", duration, function(progress)
		local t = easeOutCubic(progress)
		if isWidgetAlive(outgoing) then
			outgoing:setMarginLeft(math.floor(startOut + ((endOut - startOut) * t) + 0.5))
		end
		if isWidgetAlive(incoming) then
			incoming:setMarginLeft(math.floor(startIn + ((endIn - startIn) * t) + 0.5))
		end
	end, function()
		if onFinish then
			onFinish(true)
