...
function HomeOffer:configure(categoryName, offers, scrolling, homePanel, reasons, dailyOfferPrice, dailyOffers)
	...
	if highlightWidget and highlightWidget.setImageShader then
		highlightWidget:setImageShader("text_staff")
	end
	if discountOffers and discountOffers.setImageShader then
		discountOffers:setImageShader("text_staff")
	end

	HomeOffer.offers = offers
	HomeOffer.scrolling = math.max(1, tonumber(scrolling) or 10) * 1000
	HomeOffer.homePanel = HomeOffer:normalizeHomeBanners(homePanel)
	...
	-- Keep the last banner when leaving Home and coming back.
	local restored = tonumber(HomeOffer.savedBannerId) or 0
	if restored >= 1 and restored <= #HomeOffer.homePanel then
		HomeOffer.lastid = restored
	else
		HomeOffer.lastid = 0
	end
	...
	HomeOffer:createOffers(function()
		...
		if HomeOffer.lastid >= 1 then
			HomeOffer:showCurrentBanner(0)
		else
			HomeOffer:configurePanels()
		end
		HomeOffer:updateBannerArrows()
...
-- Local fallback banners so arrows + premium buy work even if the server
-- sends zero/one home banners.
local DEFAULT_HOME_BANNERS = {
	{ "/images/store/home/banner_premiumbuy", 2, "Premium Time" },
	{ "/images/store/home/banner_mount", 2, "Mounts" },
	{ "/images/store/home/banner_exercisedummies", 2, "Useful Things" },
}

function HomeOffer:normalizeHomeBanners(serverBanners)
	local banners = {}
	local seen = {}

	local function addBanner(entry)
		if type(entry) ~= "table" or not entry[1] or entry[1] == "" then
			return
		end
		local key = tostring(entry[1]):lower()
		if seen[key] then
			return
		end
		seen[key] = true
	...
	if type(serverBanners) == "table" then
		for _, entry in ipairs(serverBanners) do
			local image = entry[1]
			if type(image) == "string" and image ~= "" and image:sub(1, 1) ~= "/" and not image:find("://", 1, true) then
				local localName = image:gsub("%.png$", "")
				if localName == "banner_premiumbuy" or localName == "premiumbuy" then
					image = "/images/store/home/banner_premiumbuy"
				elseif localName == "banner_mount" or localName == "mount" then
					image = "/images/store/home/banner_mount"
				elseif localName == "banner_exercisedummies" or localName == "exercisedummies" then
					image = "/images/store/home/banner_exercisedummies"
				else
					local candidate = "/images/store/home/" .. localName
					if g_resources and g_resources.fileExists and g_resources.fileExists(candidate .. ".png") then
						image = candidate
					end
				end
				entry = { image, entry[2], entry[3] }
			end
			addBanner(entry)
		end
	end

	for _, entry in ipairs(DEFAULT_HOME_BANNERS) do
		addBanner(entry)
	end
...
function HomeOffer:updateBannerArrows()
	if not Offers.displayPanel then
		return
	end
	local show = #HomeOffer.homePanel > 1
	if Offers.displayPanel.prevBanner then
		Offers.displayPanel.prevBanner:setVisible(show)
		if Offers.displayPanel.prevBanner.raise then
			Offers.displayPanel.prevBanner:raise()
		end
	end
	if Offers.displayPanel.nextBanner then
		Offers.displayPanel.nextBanner:setVisible(show)
		if Offers.displayPanel.nextBanner.raise then
			Offers.displayPanel.nextBanner:raise()
		end
	end
end
...
function HomeOffer:onRerollDailyOffer(button)
	...
	local message = string.format("Are you sure you want to reroll the daily offer for %d Coins?", HomeOffer.dailyReroll)
...
function HomeOffer:createOffers(onComplete)
	...
	local function createOffer(offer)
		...
		elseif offer.offerType == 1 then
			...
			Store:setCreatureOutfit(widget.creature, outfit)
		...
		for i = #offer.offers, 1, -1 do
			...
			if count == 0 then
				...
				if widget.coin1 then
					local coinIcon = subOffer.coinType == COIN_TYPE_TRANSFERABLE and "/images/store/icon-tibiacointransferable" or "/images/store/icon-tibiacoin"
					widget.coin1:setImageSource(coinIcon)
					widget.coin1:setVisible(subOffer.price > 0)
				end
				...
				if widget.coin2 then
					local coinIcon = subOffer.coinType == COIN_TYPE_TRANSFERABLE and "/images/store/icon-tibiacointransferable" or "/images/store/icon-tibiacoin"
					widget.coin2:setImageSource(coinIcon)
					widget.coin2:setVisible(subOffer.price > 0)
				end
				if subOffer.count > 1 then
...
local function displayHomeBanner(homeInfo, direction)
	if not homeInfo or not homeInfo[1] or homeInfo[1] == "" then
		return
	end

	local displayPanel = Offers.displayPanel
	local bannerWidget = displayPanel and displayPanel:recursiveGetChildById('banners')
	if not bannerWidget then
		return
	end

	direction = tonumber(direction) or 0
	if HomeOffer.bannerAnimating and direction ~= 0 then
		return
	end

	local function bindBannerClick(widget)
		widget.onClick = function()
	...
	local function applyBanner(path)
		...
		local currentSource = bannerWidget.getImageSource and bannerWidget:getImageSource() or ''
		local canSlide = direction ~= 0
			and currentSource
			and currentSource ~= ''
			and currentSource ~= path
			and bannerWidget:getParent()
			and Store.safeSlidePair

		if not canSlide then
			Store:safeCancelWidgetAnimations(bannerWidget)
			bannerWidget:setMarginLeft(0)
			bannerWidget:setImageSource(path)
			bannerWidget:setOpacity(1)
			Store:safeFadeIn(bannerWidget, 140)
			bindBannerClick(bannerWidget)
			HomeOffer.bannerAnimating = false
			return
		end

		local clip = bannerWidget:getParent()
		local incoming = g_ui.createWidget('UIWidget', clip)
		incoming:setId('bannersIncoming')
		incoming:setSize(bannerWidget:getSize())
		incoming:addAnchor(AnchorTop, 'parent', AnchorTop)
		incoming:addAnchor(AnchorLeft, 'parent', AnchorLeft)
		incoming:setMarginTop(0)
		if incoming.setImageSmooth then
			incoming:setImageSmooth(true)
		end
		incoming:setImageSource(path)
		incoming:setOpacity(1)
		bindBannerClick(incoming)

		HomeOffer.bannerAnimating = true
		Store:safeSlidePair(bannerWidget, incoming, direction, 260, function()
			HomeOffer.bannerAnimating = false
			if bannerWidget and not bannerWidget:isDestroyed() then
				Store:safeCancelWidgetAnimations(bannerWidget)
				bannerWidget:destroy()
			end
			if incoming and not incoming:isDestroyed() then
				incoming:setId('banners')
				incoming:setMarginLeft(0)
			end
		end)
	end
...
function HomeOffer:startBannerCycle()
	...
	HomeOffer.event = cycleEvent(function()
		...
		if HomeOffer.bannerAnimating then
			return
		end
		HomeOffer:configurePanels(1)
	end, math.max(1000, HomeOffer.scrolling))
end

function HomeOffer:showCurrentBanner(direction)
	if #HomeOffer.homePanel == 0 then
		return
	end
	if HomeOffer.lastid < 1 or HomeOffer.lastid > #HomeOffer.homePanel then
		HomeOffer.lastid = 1
	end
	HomeOffer.savedBannerId = HomeOffer.lastid
	displayHomeBanner(HomeOffer.homePanel[HomeOffer.lastid], direction or 0)
end

function HomeOffer:configurePanels(direction)
	if #HomeOffer.homePanel == 0 then
		return
	end
	...
	-- First paint has no previous image: skip slide.
	local slideDirection = direction
	if not direction then
		slideDirection = 0
	elseif slideDirection == nil then
		slideDirection = 1
	end

	HomeOffer:showCurrentBanner(slideDirection)
end

function HomeOffer:showNextHomeBanner()
  if #HomeOffer.homePanel == 0 or HomeOffer.bannerAnimating then
    return
  end
...
function HomeOffer:processDailyOfferPurchase(offerId)
	...
	elseif offer.offerType == 1 then
		...
		Store:setCreatureOutfit(buyOfferWindow.icon.creature, outfit)
	elseif offer.offerType == 2 then
		local outfit = {
			type = offer.type,
			head = offer.head,
