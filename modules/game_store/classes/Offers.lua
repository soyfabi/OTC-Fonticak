...
function Offers:onSelectionOffer(_, selectedWidget)
	...
	elseif offer.offerType == CATEGORY_MOUNT then
		...
		Store:setCreatureOutfit(Offers.displayPanel.infopanel.outfit, outfit)
	elseif offer.offerType == CATEGORY_OUTFIT then
		local outfit = {
			type = offer.type,
			head = offer.head,
	...
	Offers.displayPanel.price1.image:setVisible(offer.offers[1].price > 0)
	...
	if #offer.offers > 1 then
		...
		Offers.displayPanel.price2.image:setVisible(offer.offers[2].price > 0)
	...
	if offer.bundles and #offer.bundles > 0 then
		...
		for i, bundles in pairs(offer.bundles) do
			...
			if bundles.offerType == 3 then
				...
				Store:setCreatureOutfit(ui.creature, { auxType = bundles.itemId})
				ui.name:setText(bundles.name)
			elseif bundles.offerType == 1 then
				local ui = g_ui.createWidget('CreatureLabel', Offers.displayPanel.description.package)
				Store:setCreatureOutfit(ui.creature, {type = bundles.mountId})
				ui.name:setText(bundles.name)
			elseif bundles.offerType == 2 then
				local ui = g_ui.createWidget('CreatureLabel', Offers.displayPanel.description.package)
				Store:setCreatureOutfit(ui.creature, {
					type = bundles.type,
					head = bundles.head,
					body = bundles.body,
					legs = bundles.legs,
