...
function StatsBar.onHarmonyChange(localPlayer, harmony)
    modules.game_healthcircle.whenMonkHarmonyChange(localPlayer, harmony)
    local statsBars = StatsBar.getAllStatsBarWithPosition()
    for _, barElement in ipairs(statsBars) do
        local harmonies = barElement:recursiveGetChildById('harmonies')
        if harmonies then
            local tooltip = tr('%d/5 Harmony\n\nHarmony is generated by specific abilities of your character.', math.min(5, harmony))
            for i = 1, 5 do
...
function StatsBar.onSereneChange(localPlayer, serene)
    modules.game_healthcircle.whenMonkSereneChange(localPlayer, serene)
    local statsBars = StatsBar.getAllStatsBarWithPosition()
    for _, barElement in ipairs(statsBars) do
...
function StatsBar.refreshMonkState(localPlayer)
    if not localPlayer then
        return
    end

    StatsBar.onVocationChange(localPlayer, localPlayer:getVocation())
    StatsBar.onHarmonyChange(localPlayer, localPlayer:getHarmony())
    StatsBar.onSereneChange(localPlayer, localPlayer:isSerene())
end

showHarmonyTopBar = true

function StatsBar.setHarmonyVisible(visible)
    showHarmonyTopBar = visible ~= false
    local player = g_game.getLocalPlayer()
    if player then
        StatsBar.applyMonkStatsVisibility(player)
    end
end

function StatsBar.applyMonkStatsVisibility(localPlayer)
    local isMonk = isMonkPlayer(localPlayer)
    local statsBars = StatsBar.getAllStatsBarWithPosition()
    for _, barElement in ipairs(statsBars) do
        local monkStats = barElement:recursiveGetChildById('monkStats')
        if monkStats then
            if isMonk and showHarmonyTopBar then
                monkStats:enable()
            else
                monkStats:disable()
            end
...
function StatsBar.onVocationChange(localPlayer, vocation, oldVocation)
    ...
    StatsBar.applyMonkStatsVisibility(localPlayer)
end
