...
-- =           Rotation engine                   =
-- =============================================*/
local function findNextAvailableAction(multiActions, button)
    ...
    local closestCooldownActionIndex = nil
    local closestCooldownTime = math.huge
    local firstValidAction = nil
    local firstValidActionIndex = nil
    local onlyOneSpell = true
    local firstSpellData = nil
    local spellCount = 0

    local lastIndex = button and button.cache and button.cache.lastMultiIndex or 0

    for step = 1, 3 do
        local i = (lastIndex + step - 1) % 3 + 1
    ...
    elseif closestCooldownAction then
        return closestCooldownAction, closestCooldownActionIndex
    else
        return firstValidAction, firstValidActionIndex
    end
end
...
-- =============================================*/
local function renderSlotOnWidget(widget, slotData, isMainButton, slotIndex)
    if not widget or not slotData or table.empty(slotData) then
        return
    end

    if isMainButton then
        widget.cache.activeMultiIndex = slotIndex
    end
...
function updateMultiButtonState(button)
    ...
    -- Early return: already displaying this action
    if button.cache.activeMultiIndex == actionIndex then
    ...
    removeCooldown(button)
    renderSlotOnWidget(button, action, true, actionIndex)
    ...
    for k = 1, 3 do
        ...
        if actionButton then
            ...
            if actionButton.item then
                ...
                actionButton.item.onClick = function()
                    if actionButton.parentButton and actionButton.parentButton.cache then
                        actionButton.parentButton.cache.lastMultiIndex = k
                    end
                    onExecuteAction(actionButton)
                    closeCurrentMultiActionPanel()
                end
                if actionButton.item.text then
                    actionButton.item.text.onClick = function()
                        if actionButton.parentButton and actionButton.parentButton.cache then
                            actionButton.parentButton.cache.lastMultiIndex = k
                        end
                        onExecuteAction(actionButton)
